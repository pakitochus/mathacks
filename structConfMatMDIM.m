function [performance,confMat] = structConfMatMDIM(perf)
...
% dimension or non-binary classifiers are allowed. 
    ...
    performance = struct();
    yu=mat2cell(confMat,[sqrt(numel(confMat))/2 sqrt(numel(confMat))/2],[sqrt(numel(confMat))/2 sqrt(numel(confMat))/2]);
    confMat=cellfun(@(x) mean(x(:)),yu);
    N = sum(confMat(:)); 
